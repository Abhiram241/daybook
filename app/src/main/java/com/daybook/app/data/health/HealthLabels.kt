...
/** §7.1 — `HealthSession.exerciseType` is the raw Health Connect int; this maps it to a display
 *  label, unit-tested for totality via its `[key] ?: "Workout"` fallback so an unknown/future
 *  type renders as "Workout" instead of crashing. */
object ExerciseTypeLabels {
    private val MAP: Map<Int, String> = mapOf(
        ...
        ExerciseSessionRecord.EXERCISE_TYPE_FOOTBALL_AMERICAN to "American football",
    ...
    fun labelFor(exerciseType: Int): String = MAP[exerciseType] ?: "Workout"
}

/** §6.1.6 — resolves `metadata.dataOrigin.packageName` to a human "From X" label for the
 *  Nutrition card. Unrecognised packages fall back to "From another app", never a raw package id. */
object SourceAppLabels {
    private val MAP: Map<String, String> = mapOf(
        ...
        "com.zepp.fitness" to "Zepp"
    )

    /** Returns null when [packageName] is null (no attribution available at all) — the caller
     *  decides the "From …" copy, including the "From another app" fallback for a non-null but
     *  unrecognised package. */
    fun labelFor(packageName: String?): String? =
        packageName?.let { MAP[it] ?: "another app" }
}
